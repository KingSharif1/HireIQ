...
Application tracking
    ...
    → Fixed-job Documents editor: tailored snapshot (pen-edit text + include/exclude) → Design (mobile size/order) → Match highlights on preview → tailored_resumes
...
| 3.3 Resume build | ATS keywords in real bullets + recruiter voice | `lib/ai/tailor-pipeline.ts`, `tailor-engine.ts` | ✓ One rewrite; no critique loop |
